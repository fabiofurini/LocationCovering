...
//	if(inst->option==2 || inst->option==10)
//	{
//
//		////////////////////////////////////////////////////////////
//		load_I_TILDE_CFL(inst,true,I_TILDE_BEN_2,_cut_CFL_BEN_2_Y);
//		comb_solve_model_CFL_BEN_2_AUX_1(inst,I_TILDE_BEN_2,AUX_SOL_2);
//		////////////////////////////////////////////////////////////
//
//
//#ifdef DEBUG
...
//	if(inst->option==3 || inst->option==10)
//	{
...
/*****************************************************************/
{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef USE_CUT_LOOP
	inst->status = CPXchgprobtype (inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2, CPXPROB_LP);
	bool give_up=false;

	double *Y_ZERO=new double[inst->n_locations];
	double *Y_TILDE=new double[inst->n_locations];

	double *I_TILDE=new double[inst->n_clients];
	...
	while(!give_up){
		...
		inst->status=CPXlpopt(inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2);
		if(inst->status!=0){printf("error in CPXmipopt\n");}

		inst->status=CPXgetx(inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2,Y_TILDE,0,inst->n_locations-1);
		if(inst->status!=0){printf("error in CPXgetmipx\n");}

		inst->status=CPXgetmipobjval(inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2,&(inst->objval));
		if(inst->status!=0){printf("error in CPXgetmipobjval\n");}

		cout << "CURRENT VALUE\t" << inst->objval << endl;
		//cin.get();

		bool give_up_2=cup_loop(inst,Y_TILDE,Y_ZERO,I_TILDE);

		//cout << "give_up_2\t" << give_up_2 << endl;

		if(give_up_2==true){give_up=true;}

		counter_rounds++;
		if(counter_rounds>=20){give_up=true;}
	}
	...
	int * indices=new int[inst->n_locations];
	char * xctype=new char[inst->n_locations];
	for ( int i = 0; i < inst->n_locations; i++ )
	{
		indices[i]=i;
		xctype[i]='B';
	...
	inst->status=CPXchgctype(inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2, inst->n_locations,indices,xctype );
	if(inst->status!=0)
	{
		printf("error in CPXaddrows\n");
		exit(-1);
...
//	}
	...
	CPXsetintparam (inst->env_CFL_BEN_2, CPX_PARAM_SCRIND, CPX_OFF);
	...
	inst->status=CPXgetobjval(inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2,&cplex_lp);
	if(inst->status!=0)
	{
		printf("error in CPXgetmipobjval\n");
		exit(-1);
	...
	printf("\n\nLP solution value ->\t\%f -- time \%f",cplex_lp,solution_time_lp);

	inst->status=CPXgetx(inst->env_CFL_BEN_2,inst->lp_CFL_BEN_2,inst->x,0,inst->n_locations-1);
	if(inst->status!=0)
	{
		printf("error in CPXgetmipx\n");
		exit(-1);
...
#ifdef print_solution_lp
	printf("\n\nSolution LP\n");
	for ( int j = 0; j < inst->n_locations; j++){

		cout << "Location\t" << j << "\tval:\t" << inst->x[position_y_CFL_BEN_2(inst,j)] << endl;
