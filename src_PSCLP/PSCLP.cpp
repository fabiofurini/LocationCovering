...
//			for ( int j = 0; j < inst->n_locations; j++){
//				yy[j]= (int)(inst->x[position_y_CFL(inst,j)]+0.5);
//			}
//			for ( int i = 0; i < inst->n_clients; i++ ){
//				zz[i]=(int)(inst->x[position_z_CFL(inst,i)]+0.5);
//			}
//
...
//		/////////////////////////////////////////////////////////////
	...
	inst->status = CPXchgprobtype (inst->env_CFL, inst->lp_CFL, CPXPROB_LP);

	clock_t time_start_lp=clock();
	cout << "\nCPXlpopt:\n";
	inst->status=CPXlpopt(inst->env_CFL,inst->lp_CFL);
	if(inst->status!=0)
	{
	...
	inst->status=CPXgetobjval(inst->env_CFL,inst->lp_CFL,&cplex_lp);
	if(inst->status!=0)
	{
		printf("error in CPXgetmipobjval\n");
		exit(-1);
	...
	inst->status=CPXgetx(inst->env_CFL,inst->lp_CFL,inst->x,0,inst->n_locations+inst->n_clients-1);
	if(inst->status!=0)
	{
		printf("error in CPXgetmipx\n");
		exit(-1);
	...
	inst->pi=(double*) calloc(1+inst->n_clients,sizeof(double));

	inst->status=CPXgetpi(inst->env_CFL,inst->lp_CFL,inst->pi,0,1+inst->n_clients-1);
	if(inst->status!=0)
	{
		printf("error in CPXgetmipx\n");
		exit(-1);
	...
	//	cout << "DUAL VARIABLES\n";
	//	for ( int kk = 0; kk <= inst->n_clients; kk++){
	//		cout << "constraint dual var\t" << kk << "\t" << inst->pi[kk] << endl;
	//	}

	cout << "\n\n***dual var of covering constraint\t" << inst->pi[inst->n_clients] << endl;
	...
	for ( int j = 0; j < inst->n_locations; j++){
		...
		cout << "Location\t" << j << "\tval:\t" << inst->x[position_y_CFL(inst,j)] << endl;

	}
	printf("\n");
	for ( int i = 0; i < inst->n_clients; i++ ){

		cout << "Client\t" << i << "\tval:\t" << inst->x[position_z_CFL(inst,i)] << endl;
	}
	printf("\n");
#endif
